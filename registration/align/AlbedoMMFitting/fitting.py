...
class FlameFitter:
    ...
    def _load_dataset(self):
        ...
        meta_file_path = os.path.join(self.config["coarse_fitting_root"], "align_scaled.json")
        ...
        self.height, self.width = int(meta['h']), int(meta['w'])
    ...
    def render_img(self, vertices, texture):
        ...
        return (normal_img + 1) / 2

    def optimize_landmarks(self, iters=1000, log_freq=250):
        init_align = torch.load(
            os.path.join(self.config["coarse_fitting_root"], "align_init.pkl")
        )
        
        # create params
        bz = 1
        scale = torch.ones(1).to(self.device)
        R = matrix_to_rotation_6d(
            torch.from_numpy(init_align["rot"]).float()[None, ...]  # [1,3,3]
        ).to(self.device)
        t = torch.from_numpy(init_align["trans"]).float().to(self.device)
        # shape = self.mica_shape
        shape = torch.zeros(bz, self.config.num_shape_params).to(self.device)
    ...
    def render_specular(self, res):
        meta_file_path = os.path.join(self.data_root, "transforms_select_flame.json")
        with open(meta_file_path, 'r') as f:
            meta = json.load(f)
